...
object TicTacToeApp {
  ...
  object GameState {
    implicit val showGameState: Show[GameState] = Show.show(gs =>
     s""" Turn : ${gs.turn}
        |
        |
        |
        ...
        |
        |
        |Plays: ${gs.plays.reverse}""".stripMargin )
  }
  ...
  class TicTacToe(boardSize: Int) {
    ...
    def doPlay(play: Play): Game[Unit] = {
      State.modify[GameState] { gameState =>
        ...
        val playsUpdated = play :: gameState.plays 
        gameState.copy(board = newBoard, plays = playsUpdated)
      }
    }
    ...
    def game: State[GameState, (Player, List[Play])] = {
      for {
        ...
        plays <- gameState.plays.pure[Game]
        winner        <- if(weHaveAWinner) gameState.turn.pure[Game]
                         else switchTurns >> game.map(_._1)
      } yield (winner, plays)
    }

    def playGame: (GameState, (Player, List[Play])) = game.run(
      initialState
    ).value

    def replayGame(plays: List[Play]):GameState = {
      def replayGameSt(plays: List[Play]): Game[GameState]= {
             plays match {
              case head :: next => doPlay(head) >> switchTurns >> replayGameSt(next)
              case Nil =>  State.get[GameState]
             }
      }
      replayGameSt(plays).runS(initialState).value
    }
  }
