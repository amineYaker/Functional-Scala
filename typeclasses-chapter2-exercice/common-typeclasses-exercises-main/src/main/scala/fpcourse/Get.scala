...
object Get {
   ...
   * 
   */
  def skip(n: Int): Get[Unit] =  Get[Unit](list => if (list.size < n) Left("input too small to skip") else Right(list.drop(n), ()))

  /**
   * TODO 2
   * True if the input is fully consumed
   */
  def isEmpty: Get[Boolean] = Get(list => Right(list, list.isEmpty))

  /**
   * TODO 3
   * Reads one byte from input
   */
  def getByte: Get[Byte] = Get[Byte](bytes => bytes match {
    case head :: tl => Right(tl, head)
    case Nil => Left("empty input")
  })
   ...
   */
  def getString(n: Int): Get[String] = getByte.replicateA(n).map( bytes => new String(bytes.toArray))
