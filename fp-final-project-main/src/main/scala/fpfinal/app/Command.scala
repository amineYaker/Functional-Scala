...
import fpfinal.service.LivePersonService
  ...
  def execute(): AppOp[SuccessMsg] = {
    ...
    def readParticipants(): AppOp[List[String]] = {
      ME.tailRecM(List[String]()) { xs: List[String] =>
        val msg: String =
          "Please enter the name of the participant or write END to finish"
        for {
          env <- readEnv
    ...
    def validateData(
      ...
      ).mapN((p, am, parts) => AddExpenseData(p, am, parts))
    }
    ...
    def findPerson(name: String): AppOp[Person] =
      for {
        ...
        person <- ME.fromOption(personOpt, s"person $name not found")
      } yield (person)
